...
-- Partial unique indexes: allow multiple movies with tmdb_id=0 or empty imdb_id
CREATE UNIQUE INDEX IF NOT EXISTS idx_movies_tmdb_id_unique ON movies(tmdb_id) WHERE tmdb_id > 0;
CREATE UNIQUE INDEX IF NOT EXISTS idx_movies_imdb_id_unique ON movies(imdb_id) WHERE imdb_id IS NOT NULL AND imdb_id != '';
