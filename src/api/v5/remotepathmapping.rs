...
pub async fn create_remote_path_mapping(Json(body): Json<RemotePathMappingResource>) -> Json<RemotePathMappingResource> {
    Json(body)
}

pub async fn update_remote_path_mapping(Path(id): Path<i32>, Json(mut body): Json<RemotePathMappingResource>) -> Json<RemotePathMappingResource> {
    body.id = id;
    Json(body)
}
...
pub fn routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/{id}", get(get_remote_path_mapping).put(update_remote_path_mapping).delete(delete_remote_path_mapping))
}
