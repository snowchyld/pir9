...
pub async fn create_custom_filter(Json(body): Json<CustomFilterResource>) -> Json<CustomFilterResource> {
    Json(body)
}

pub async fn update_custom_filter(Path(id): Path<i32>, Json(mut body): Json<CustomFilterResource>) -> Json<CustomFilterResource> {
    body.id = id;
    Json(body)
}
