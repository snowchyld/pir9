...
use crate::core::datastore::repositories::{EpisodeRepository, MovieRepository, SeriesRepository};
...
pub async fn get_wanted_missing(
    ...
    let exclude_ids: Vec<i64> = state
        .tracked
        .get_all_any()
        .await
        .iter()
        .flat_map(|td| td.episode_ids.iter().copied())
        .collect();
