...
/// GET /api/v3/tag
pub async fn get_tags(State(state): State<Arc<AppState>>) -> Json<Vec<TagResource>> {
...
pub async fn get_tag_details(State(state): State<Arc<AppState>>) -> Json<Vec<TagDetailsResource>> {
    ...
    let resources: Vec<TagDetailsResource> = tags
        ...
        .map(|(id, label)| {
            ...
            TagDetailsResource {
                ...
                download_client_ids,
                auto_tag_ids: vec![],
                import_list_ids: delay_profiles
                    .iter()
...
pub async fn get_tag_detail(
    ...
    Json(Some(TagDetailsResource {
        ...
        indexer_ids: indexers
            ...
            .map(|i| i.id as i32)
            .collect(),
        download_client_ids: download_clients
            .iter()
            .filter(|dc| parse_tags(&dc.tags).contains(&tag_id))
            .map(|dc| dc.id as i32)
            .collect(),
