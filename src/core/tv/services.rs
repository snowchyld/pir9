...
impl SeriesService {
    ...
    pub async fn add_series(&self, options: AddSeriesOptions) -> Result<Series> {
        ...
        self.apply_monitoring_options(series.id, &options.add_options)
            .await?;
    ...
    pub async fn refresh_series(&self, series_id: i64, force: bool) -> Result<()> {
        ...
        // Publish event
        self.event_bus
            .publish(Message::SeriesRefreshed {
                series_id,
                title: series.title,
            })
            .await;
    ...
    async fn apply_monitoring_options(&self, series_id: i64, options: &AddOptions) -> Result<()> {
        ...
        for mut episode in episodes {
            let should_monitor = match options.monitor {
                ...
                NewItemMonitorType::Future => episode
                    .air_date_utc
                    .map(|d| d > Utc::now())
                    .unwrap_or(false),
    ...
    async fn search_for_missing(&self, series_id: i64) -> Result<()> {
        ...
        for episode in episodes {
            self.event_bus
                .publish(Message::EpisodeSearchRequested {
    ...
    pub async fn delete_series(&self, id: i64, delete_files: bool) -> Result<()> {
        ...
        if delete_files {
            // Delete series files
            tokio::fs::remove_dir_all(&series.full_path())
                .await
    ...
    async fn fetch_series_metadata(&self, tvdb_id: i64) -> Result<SeriesMetadata> {
        ...
        let first_aired = skyhook
            .first_aired
            .as_ref()
            .and_then(|s| chrono::NaiveDate::parse_from_str(s, "%Y-%m-%d").ok());

        // Convert images
        let images: Vec<crate::core::tv::MediaCover> = skyhook
            .images
            .unwrap_or_default()
            .into_iter()
            .filter_map(|img| {
        ...
        // Convert seasons
        let seasons: Vec<crate::core::tv::Season> = skyhook
            .seasons
            .unwrap_or_default()
            .into_iter()
        ...
        // Convert episodes
        let episodes: Vec<EpisodeMetadata> = skyhook
            .episodes
            .unwrap_or_default()
            .into_iter()
            .map(|ep| {
        ...
        info!(
            "Fetched {} seasons and {} episodes from Skyhook",
            seasons.len(),
            episodes.len()
        );
